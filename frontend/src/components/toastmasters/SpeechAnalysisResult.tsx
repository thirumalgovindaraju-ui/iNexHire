...
export default function SpeechAnalysisResult({ analysis, onRecordAgain, onSpeakingChange, autoPlay, agentGender, agentAccent, roleId }: {
  analysis: TmSpeechAnalysis;
  onRecordAgain: () => void;
  onSpeakingChange?: (speaking: boolean) => void;
  /** Read the transcript aloud immediately once, for an unattended "run the show" flow. */
  autoPlay?: boolean;
  /** AI agent voice persona, when this speech was generated by an agent. */
  agentGender?: TmAgentGender | null;
  agentAccent?: TmAgentAccent | null;
  /** Present only for an AI-agent speech — enables the "hold to interrupt" control. */
  roleId?: string;
}) {
  const { show, ToastContainer } = useToast();
  const speech = useSpeech(onSpeakingChange);
  const { speaking, play, stop } = speech;
  const interjection = useAgentInterjection(speech, roleId ?? '', { accent: agentAccent, gender: agentGender });
  const autoPlayedRef = useRef(false);
  useEffect(() => {
    if (autoPlay && !autoPlayedRef.current) {
      autoPlayedRef.current = true;
      play(analysis.transcript, { accent: agentAccent, gender: agentGender });
    }
