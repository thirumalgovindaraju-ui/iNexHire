...
export const timerApi = {
  list: async (meetingId: string): Promise<TmTimerLog[]> => {
    const res = await apiClient.get(`/toastmasters/${meetingId}/timer-logs`);
  ...
  submit: async (meetingId: string, data: {
    ...
    const res = await apiClient.post(`/toastmasters/${meetingId}/timer-logs`, data);
...
export const ahCounterApi = {
  list: async (meetingId: string): Promise<TmAhCounter[]> => {
    const res = await apiClient.get(`/toastmasters/${meetingId}/ah-counter`);
    return res.data.counters;
  },
  tap: async (meetingId: string, memberId: string, fillerWord: string): Promise<TmAhCounter> => {
    const res = await apiClient.post(`/toastmasters/${meetingId}/ah-counter`, { memberId, fillerWord });
  ...
  saveAll: async (meetingId: string, counters: Array<{
    ...
    const res = await apiClient.post(`/toastmasters/${meetingId}/ah-counter/bulk`, { counters });
    return res.data.counters;
  },
};
...
export const grammarianApi = {
  get: async (meetingId: string): Promise<TmGrammarianLog | null> => {
    const res = await apiClient.get(`/toastmasters/${meetingId}/grammarian`);
    return res.data.log;
  },
  upsert: async (meetingId: string, data: Partial<{
    ...
    const res = await apiClient.put(`/toastmasters/${meetingId}/grammarian`, data);
    return res.data.log;
  },
};
...
export const tableTopicsApi = {
  list: async (meetingId: string): Promise<TmTableTopicResponse[]> => {
    const res = await apiClient.get(`/toastmasters/${meetingId}/table-topics`);
  ...
  create: async (meetingId: string, data: {
    ...
    const res = await apiClient.post(`/toastmasters/${meetingId}/table-topics`, data);
  ...
  update: async (responseId: string, data: Partial<{
    ...
    const res = await apiClient.patch(`/toastmasters/table-topics/${responseId}`, data);
    return res.data.response;
  },
};
...
export const reportApi = {
  get: async (meetingId: string): Promise<TmMeetingFullReport> => {
    const res = await apiClient.get(`/toastmasters/${meetingId}/report`);
    return res.data.report;
  },
  generate: async (meetingId: string, data?: { bestSpeakerRoleId?: string; bestTableTopicId?: string; bestEvaluatorRoleId?: string }) => {
    const res = await apiClient.post(`/toastmasters/${meetingId}/report/generate`, data ?? {});
    return res.data.report;
  },
};
