...
export default function Compliance() {
  ...
  const [selected, setSelected]     = useState<BiasAudit | null>(null);
  const [loading, setLoading]       = useState(true);
  const [scanning, setScanning]     = useState(false);
  const [resolving, setResolving]   = useState(false);
  const [error, setError]           = useState<string | null>(null);

  // ── Load all audits for this org ──────────────────────────────────────────
  const fetchAudits = useCallback(async () => {
    try {
      ...
      if (data.audits?.length > 0 && !selected) {
        setSelected(data.audits[0]);
      }
    } catch (err: any) {
  ...
  async function runScan() {
    ...
    try {
      setScanning(true);
      setError(null);
      const { data } = await apiClient.post(`/compliance/${selected.openingId}`, {
        scanType: 'jd',
      });
      // Update just this entry in the list
      setAudits((prev) =>
        prev.map((a) =>
          a.openingId === selected.openingId
            ? { ...a, ...data.audit, openingTitle: a.openingTitle }
            : a
        )
      );
      setSelected((prev) =>
        prev ? { ...prev, ...data.audit } : data.audit
      );
    } catch (err: any) {
  ...
  async function resolveAudit() {
    ...
    try {
      ...
      const { data } = await apiClient.patch(
        `/compliance/${selected.openingId}/resolve`
      );
      setAudits((prev) =>
        prev.map((a) =>
          a.openingId === selected.openingId
            ? { ...a, resolvedAt: data.audit.resolvedAt }
            : a
        )
      );
      setSelected((prev) => (prev ? { ...prev, resolvedAt: data.audit.resolvedAt } : prev));
    } catch (err: any) {
  ...
  const totalFlags   = audits.reduce((s, a) => s + (a.flags?.length ?? 0), 0);
  const cleanCount   = audits.filter((a) => (a.flags?.length ?? 0) === 0).length;
  ...
  return (
    <div style={S.page}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 24 }}>
        ...
        <Button icon={<RefreshCw size={14} />} onClick={runScan} loading={scanning} disabled={!selected}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 20 }}>
        {[
          { label: 'JDs Scanned',      value: String(audits.length), icon: <FileText size={16} />, color: '#4f46e5', bg: '#eef2ff' },
      ...
      {audits.length === 0 ? (
        <div style={{ ...S.card, padding: 40, textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 15, fontWeight: 600, color: '#0f172a', marginBottom: 6 }}>No scans yet</div>
          <div style={{ fontSize: 13, color: '#94a3b8' }}>
            Open any job posting and click "Run Scan" to analyse it for bias.
          </div>
        </div>
      ) : (
          ...
          <div style={S.card}>
            ...
            <div>
              {audits.map((a) => {
                ...
                const isActive  = selected?.openingId === a.openingId;
                return (
                  <div
                    key={a.openingId}
                  ...
                  >
                    <span style={{ fontWeight: isActive ? 500 : 400 }}>{a.openingTitle}</span>
                    {flagCount > 0
                      ? <span style={{ background: '#fff1f2', color: '#f43f5e', fontSize: 9, borderRadius: 9, padding: '2px 6px', fontWeight: 600 }}>{flagCount} flags</span>
            ...
            {selected ? (
              <>
                <div style={{ padding: '14px 18px', borderBottom: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <div>
                    <div style={{ fontSize: 14, fontWeight: 600, color: '#0f172a' }}>{selected.openingTitle}</div>
                    <div style={{ fontSize: 11, color: '#94a3b8' }}>
                      Last scanned {new Date(selected.createdAt).toLocaleDateString()}
                      {selected.resolvedAt && ' · Resolved'}
                    </div>
                  </div>
                  {(selected.flags?.length ?? 0) > 0 && !selected.resolvedAt && (
                    <span style={{ marginLeft: 'auto', background: '#fff1f2', color: '#f43f5e', fontSize: 11, borderRadius: 6, padding: '3px 10px', fontWeight: 500 }}>
                      {selected.flags.length} {selected.flags.length === 1 ? 'issue' : 'issues'} found
                    </span>
                  )}
                  {selected.resolvedAt && (
                  ...
                  {(selected.flags?.length ?? 0) > 0 && !selected.resolvedAt ? (
                    ...
                    <div style={{ textAlign: 'center', padding: '40px 0' }}>
                      ...
                      <div style={{ fontSize: 15, fontWeight: 600, color: '#0f172a', marginBottom: 6 }}>
                        {selected.resolvedAt ? 'Issues resolved' : 'No bias detected'}
                      </div>
                      <div style={{ fontSize: 13, color: '#94a3b8' }}>
                        {selected.resolvedAt
