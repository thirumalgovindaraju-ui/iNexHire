...
// Drop-in hook for InterviewRoom.tsx — detects malpractice signals during an interview.
...
export function useProctoring({ interviewId, enabled = true }: ProctoringConfig) {
  ...
  const faceCheckInterval = useRef<ReturnType<typeof setInterval>>();
  ...
  // Matches the real backend route: POST /api/interviews/:id/proctor-event,
  // body { eventType, metadata } — see backend/src/routes/interview.routes.ts.
  const flushEvents = useCallback(async () => {
    ...
    for (const event of batch) {
      try {
        await apiClient.post(`/interviews/${interviewId}/proctor-event`, {
  ...
  // Skin-tone heuristic fallback (no ML model bundled) — good enough to flag
  // "no face in frame" / "two faces in frame" / "large shift in face position".
  ...
  // ── Audio analysis for background voices / mic silence ─────────────────
